...
export class VerifyEmailComponent {
  ...
  async submit() {
    ...
    try {
      const sessionInfo = await firstValueFrom(this.patientService.verifyInvitation(this.token, email!));
      console.log('Session Info:', sessionInfo);
      this.toast.show({ message: 'Email verified successfully!', type: 'success' });
...
this.playTestService.init(sessionInfo);
      ...
      console.error(err);
      this.toast.show({ message: 'Invalid email or expired link.', type: 'error' });
    }
  }
}